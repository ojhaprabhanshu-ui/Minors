...
const CHECK_ITEMS = [
  ...
  },
  {
    ...
    description: "Required for integrity monitoring",
    icon: "fa-desktop",
    color: "secondary",
    required: true,
  },
...
export default function SystemCheckPage({ onComplete }) {
  ...
  const runNetworkCheck = async () => {
    try {
      ...
      updateCheck(
        "network",
        passed,
        passed ? `✓ Verified (${Math.round(latency)}ms)` : "⚠ Unstable Connection"
      );
    } catch (err) {
  ...
  const requestScreenShare = async () => {
    try {
      ...
      videoTrack.onended = () => {
        updateCheck("screenShare", false, "⚠ Sharing Stopped");
      };
      updateCheck("screenShare", true, "✓ Screen Share Active");
      setErrorMsg("");
    } catch (err) {
      updateCheck("screenShare", false, "✕ Permission Required");
      setErrorMsg("Screen sharing permission was denied. Please allow screen sharing to continue.");
    }
  };
      ...
      >
        <div className="text-center mb-4 pb-3 border-bottom">
          <span
            ...
            style={{ backgroundColor: "#fef3c7", color: "#92400e", fontSize: "0.85rem" }}
          >
            System Compatibility
          </span>
          <h2 className="fw-bold text-dark mb-1">Let's make sure your device is ready for the HR interview.</h2>
          <p className="text-muted small mb-0">Complete all mandatory checks to enter the HR interview room.</p>
        </div>
        ...
        {allRequiredPassed && (
          <div
            className="p-3 mb-3 rounded-3 text-center fw-bold"
        ...
        >
          {running ? (
            ...
            <span>
              Enter HR Interview <i className="fa-solid fa-arrow-right ms-2"></i>
            </span>
          )}
        </button>
      </div>
